...
from utils.load_utils import load_m3ed_traj, m3ed_evs_iterator
...
def evaluate(config, args, net, train_step=None, datapath="", split_file=None, 
    ...
    dataset_name = "m3ed_evs"
    assert side == "left" or side == "right"
    assert stride == 1 # != does not work yet

    if config is None:
        config = cfg
        config.merge_from_file("config/eval_m3ed.yaml")
    ...
    for i, scene in enumerate(scenes):
        ...
        for trial in range(trials):
            ...
            traj_est, tstamps, flowdata = run_voxel(datapath_val, config, net, viz=viz, 
                                          iterator=m3ed_evs_iterator(datapath_val, side=side, stride=stride, timing=timing, H=H, W=W),
                                          timing=timing, H=H, W=W, viz_flow=viz_flow)

            # load traj
            tss_traj_us, traj_hf = load_m3ed_traj(datapath_val)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--config', default="config/eval_m3ed.yaml")
    parser.add_argument('--datapath', default='', help='path to dataset directory')
    parser.add_argument('--weights', default="DEVO.pth")
    parser.add_argument('--val_split', type=str, default="splits/m3ed/m3ed_val.txt")
    ...
    print("Running eval_M3ED_evs.py with config...")
    print(cfg) 
